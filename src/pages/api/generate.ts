...
import { verifySignature, basicAuth } from '@/utils/auth';
...
export const post: APIRoute = async (context) => {
    if (!basicAuth(context.request.headers)) {
        return new Response('Unauthorized', { headers: { 'WWW-Authenticate': 'Basic realm="User Login"' }, status: 401 });
    }
